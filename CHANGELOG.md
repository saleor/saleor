...
- The default ordering of attributes is now `slug` alone. It used to be `storefront_search_position` first, one of the removed presentation flags; queries that don't pass an explicit `sortBy` may return attributes in a different order than before.
