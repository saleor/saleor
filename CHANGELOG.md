...
- Added `promoCodeDetails` to `CheckoutError`, `OrderError` and `OrderCreateFromCheckoutError`, reporting a `PromoCodeRejectionReason` for a rejected voucher or gift card code so a storefront can explain the rejection instead of showing a generic message. Populated by `checkoutAddPromoCode`, `checkoutComplete`, `orderCreateFromCheckout`, `draftOrderCreate`, `draftOrderUpdate`, `orderUpdate` and `draftOrderComplete`. Reasons that would let a caller confirm a code they cannot otherwise see are reported as `NOT_FOUND`, so a rejected code stays indistinguishable from an unknown one; the reason never depends on the caller's permissions.
