...
- Removed the deprecated webhook secret key, deprecated in 3.5. The `secretKey` field of the `Webhook` type and the `secretKey` input of `WebhookCreateInput` and `WebhookUpdateInput` are gone, and the stored value is no longer used: every webhook payload is now signed with a verifiable JWS carried in the `Saleor-Signature` header. Webhooks that still had a secret key set were signing with an HMAC-SHA256 hex digest instead — those apps must switch to verifying the JWS against the Saleor instance's JWKS (`/.well-known/jwks.json`). The database column is kept for now and will be dropped in a later release.
