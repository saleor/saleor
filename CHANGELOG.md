...
- Added `idempotencyKey` field to the `TransactionChargeRequested`, `TransactionRefundRequested` and `TransactionCancelationRequested` subscription payloads. Saleor's delivery retries of a requested action carry the same key, so an app can pass it to its payment provider to avoid performing the action twice.
