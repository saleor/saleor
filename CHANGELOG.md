...
- `Order.paymentStatus` and `Order.chargeStatus` now reflect the refunds made through the transaction system, including the ones the payment app is still processing. Such an order reports `PARTIALLY_REFUNDED` or `FULLY_REFUNDED`, and stops being charged once nothing is left, instead of staying `FULLY_CHARGED` and `FULL`. - #19587 by @vojtesa
