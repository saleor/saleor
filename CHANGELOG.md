...
- Removed the observability webhook feature. The `OBSERVABILITY` webhook event and the `MANAGE_OBSERVABILITY` permission no longer exist and have been dropped from the schema (`PermissionEnum`, `WebhookEventTypeEnum`, `WebhookEventTypeAsyncEnum`, and `WebhookSampleEventEnum`). App manifests requesting `MANAGE_OBSERVABILITY` will be rejected on install, and existing subscriptions to the `OBSERVABILITY` event are removed by a migration. Use OpenTelemetry for observability instead.
