...
- Removed the deprecated available-for-purchase `Date` fields. Use `availableForPurchaseAt` (`DateTime`) instead; it holds the same date, with the time of day included.
  - `Product.availableForPurchase` -> `Product.availableForPurchaseAt`
  - `ProductChannelListing.availableForPurchase` -> `ProductChannelListing.availableForPurchaseAt`
  - `ProductChannelListingAddInput.availableForPurchaseDate` (the `updateChannels` input of `productChannelListingUpdate`) -> `ProductChannelListingAddInput.availableForPurchaseAt`

  In `productChannelListingUpdate`, setting an available-for-purchase date while `isAvailableForPurchase` is `false` returned an `INVALID` error on the `availableForPurchaseDate` field, even when the date was sent in `availableForPurchaseAt`. It is now returned on the `availableForPurchaseAt` field.
