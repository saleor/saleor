...
@pytest.fixture
def payment_credit_card_on_delivery(db, order_with_lines):
    return Payment.objects.create(
        gateway="yebofresh.payments.credit_card_on_delivery",
...
@pytest.fixture
def credit_card_on_delivery_payment_txn_preauth(order_with_lines,
                                                payment_credit_card_on_delivery):
    order = order_with_lines
    payment = payment_credit_card_on_delivery
    payment.order = order
    payment.save()
...
@pytest.fixture
def credit_card_on_delivery_payment_txn_captured(order_with_lines,
                                                 payment_credit_card_on_delivery):
    order = order_with_lines
    payment = payment_credit_card_on_delivery
    payment.order = order
...
@pytest.fixture
def credit_card_on_delivery_payment_txn_to_confirm(order_with_lines,
                                                   payment_credit_card_on_delivery):
    order = order_with_lines
    payment = payment_credit_card_on_delivery
    payment.order = order
...
@pytest.fixture
def credit_card_on_delivery_payment_txn_refunded(order_with_lines,
                                                 payment_credit_card_on_delivery):
    order = order_with_lines
    payment = payment_credit_card_on_delivery
    payment.order = order
...
@pytest.fixture
def credit_card_on_delivery_payment_not_authorized(payment_credit_card_on_delivery):
    payment_credit_card_on_delivery.is_active = False
    payment_credit_card_on_delivery.save()
    return payment_credit_card_on_delivery


@pytest.fixture(autouse=True)
def setup_credit_card_on_delivery_gateway(settings):
    settings.PLUGINS = [
        f"{gw_prefix}credit_card_on_delivery.plugin.CreditCardOnDeliveryGatewayPlugin"]
    return settings


def test_credit_card_on_delivery_authorize_success(payment_credit_card_on_delivery):
    txn = gateway.authorize(payment=payment_credit_card_on_delivery, token="Fake")
    assert txn.is_success
    assert txn.kind == TransactionKind.AUTH
    assert txn.payment == payment_credit_card_on_delivery
    payment_credit_card_on_delivery.refresh_from_db()
    assert payment_credit_card_on_delivery.is_active
...
def test_credit_card_on_delivery_authorize_failed(is_active, charge_status,
                                                  payment_credit_card_on_delivery):
    payment = payment_credit_card_on_delivery
...
def test_credit_card_on_delivery_authorize_gateway_error(
    payment_credit_card_on_delivery,
    monkeypatch
):
    monkeypatch.setattr(
        "saleor.payment.gateways.credit_card_on_delivery.credit_card_on_delivery_success",
        lambda: False
    )
    with pytest.raises(PaymentError):
        txn = gateway.authorize(payment=payment_credit_card_on_delivery, token="Fake")
...
def test_credit_card_on_delivery_void_success(
    credit_card_on_delivery_payment_txn_preauth
):
    assert credit_card_on_delivery_payment_txn_preauth.is_active
    assert credit_card_on_delivery_payment_txn_preauth.charge_status == ChargeStatus.NOT_CHARGED
    txn = gateway.void(payment=credit_card_on_delivery_payment_txn_preauth)
    assert txn.is_success
    assert txn.kind == TransactionKind.VOID
    assert txn.payment == credit_card_on_delivery_payment_txn_preauth
    credit_card_on_delivery_payment_txn_preauth.refresh_from_db()
    assert not credit_card_on_delivery_payment_txn_preauth.is_active
    assert credit_card_on_delivery_payment_txn_preauth.charge_status == ChargeStatus.NOT_CHARGED
...
def test_credit_card_on_delivery_void_failed(is_active, charge_status,
                                             payment_credit_card_on_delivery):
    payment = payment_credit_card_on_delivery
    payment.is_active = is_active
    payment.charge_status = charge_status
...
def test_credit_card_on_delivery_void_gateway_error(
    credit_card_on_delivery_payment_txn_preauth,
    monkeypatch
):
    monkeypatch.setattr(
        f"{gw_prefix}credit_card_on_delivery.credit_card_on_delivery_success",
        lambda: False)
    with pytest.raises(PaymentError):
        txn = gateway.void(payment=credit_card_on_delivery_payment_txn_preauth)
        assert txn.kind == TransactionKind.VOID
...
def test_credit_card_on_delivery_capture_success(
    amount, charge_status, credit_card_on_delivery_payment_txn_preauth):
    txn = gateway.capture(payment=credit_card_on_delivery_payment_txn_preauth,
                          amount=Decimal(amount))
    assert txn.is_success
    assert txn.payment == credit_card_on_delivery_payment_txn_preauth
    credit_card_on_delivery_payment_txn_preauth.refresh_from_db()
    assert credit_card_on_delivery_payment_txn_preauth.charge_status == charge_status
    assert credit_card_on_delivery_payment_txn_preauth.is_active
...
def test_credit_card_on_delivery_capture_failed(
    amount, captured_amount, charge_status, is_active, payment_credit_card_on_delivery
):
    payment = payment_credit_card_on_delivery
    payment.is_active = is_active
...
def test_credit_card_on_delivery_capture_gateway_error(
    credit_card_on_delivery_payment_txn_preauth,
    monkeypatch
):
    monkeypatch.setattr(
...
def test_credit_card_on_delivery_refund_success(
    initial_captured_amount,
    refund_amount,
    final_captured_amount,
    final_charge_status,
    active_after,
    credit_card_on_delivery_payment_txn_captured,
):
    payment = credit_card_on_delivery_payment_txn_captured
    payment.charge_status = ChargeStatus.FULLY_CHARGED
...
def test_credit_card_on_delivery_refund_failed(
    initial_captured_amount, refund_amount, initial_charge_status,
    payment_credit_card_on_delivery
):
    payment = payment_credit_card_on_delivery
...
def test_credit_card_on_delivery_refund_gateway_error(
    credit_card_on_delivery_payment_txn_captured,
    monkeypatch
):
    monkeypatch.setattr(
