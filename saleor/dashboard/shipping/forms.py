    ...
    class Meta:
        ...
        labels = {
            "default_weight_unit": pgettext_lazy(
                "Label of the default weight unit picker", "Default weight unit"
            )
        }
        help_texts = {
            "default_weight_unit": pgettext_lazy(
...
class ShippingZoneForm(forms.ModelForm):
    ...
    def clean_countries(self):
        ...
        if not countries:
            return
        duplicated_countries = set(countries).intersection(currently_used_countries())
    ...
    def clean(self):
        ...
        if not data.get("default") and not data.get("countries"):
            self.add_error(
                "countries",
                pgettext_lazy("ShippingZone field error", "This field is required."),
            )
        if data.get("default"):
            data["countries"] = []
        return data
...
class WeightShippingMethodForm(forms.ModelForm):
    minimum_order_weight = WeightField(
        required=False,
        label=pgettext_lazy(
            "Minimum order weight to use this shipping method", "Minimum order weight"
        ),
    )
    ...
    class Meta(ShippingMethodForm.Meta):
        fields = ["name", "price", "minimum_order_weight", "maximum_order_weight"]

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields["maximum_order_weight"].widget.attrs["placeholder"] = pgettext_lazy(
            "Placeholder for maximum order weight set to unlimited", "No limit"
        )
