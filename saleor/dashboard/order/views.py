...
def order_create(request):
    ...
    OrderEvent.draft_order_created_event(order=order, user=request.user).save()
...
def refund_payment(request, order_pk, payment_pk):
    ...
    if form.is_valid() and form.refund(request.user):
        ...
        OrderEvent.payment_refunded_event(
            order=order, user=request.user,
...
def change_fulfillment_tracking(request, order_pk, fulfillment_pk):
    ...
    if form.is_valid():
        form.save()
        events = [OrderEvent.fulfillment_tracking_updated_event(
            order=order, user=request.user,
