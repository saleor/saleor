...
def cart_index(request, cart):
    ...
    try:
        cart = Cart.objects.prefetch_related("lines__variant__product__category").get(
            pk=cart.pk
        )
...
def cart_shipping_options(request, cart):
    ...
    cart_data = get_cart_data(
        cart, shipping_price_range, request.currency, request.discounts, request.taxes
    )
    ctx.update(cart_data)
...
def update_cart_line(request, cart, variant_id):
    ...
    status = None
    form = ReplaceCartLineForm(
        request.POST, cart=cart, variant=variant, discounts=discounts, taxes=taxes
    )
    if form.is_valid():
        form.save()
        response = {
            ...
            "subtotal": 0,
            "total": 0,
            "cart": {"numItems": cart.quantity, "numLines": len(cart)},
        }
