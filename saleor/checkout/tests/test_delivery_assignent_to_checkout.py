...
def test_remove_delivery_method_from_checkout_with_shipping(
    checkout_with_item, checkout_shipping_method
):
    # given
    assigned_shipping_method = checkout_shipping_method(checkout_with_item)
    checkout_with_item.assigned_shipping_method = assigned_shipping_method
    checkout_with_item.shipping_method_name = assigned_shipping_method.name

    expected_updated_fields = {"assigned_shipping_method_id", "shipping_method_name"}
    # when
    updated_fields = remove_delivery_method_from_checkout(checkout_with_item)

    # then
    assert expected_updated_fields == set(updated_fields)
    assert checkout_with_item.assigned_shipping_method is None
...
def test_assign_shipping_to_checkout_without_delivery_method(
    ...
    assigned_shipping_method = checkout_shipping_method(checkout=checkout)
    ...
    assert checkout.assigned_shipping_method == assigned_shipping_method
    assert checkout.shipping_method_name == assigned_shipping_method.name


def test_assign_shipping_to_checkout_with_cc(
    checkout_with_delivery_method_for_cc, checkout_shipping_method
):
    # given
...
def test_assign_shipping_to_checkout_with_different_shipping_method(
    checkout, shipping_method, shipping_method_weight_based, checkout_shipping_method
):
    # given
...
def test_assign_collection_point_to_checkout_with_shipping_method(
    checkout, checkout_shipping_method, warehouses_for_cc
):
    # given
    assigned_shipping_method = checkout_shipping_method(checkout)
    checkout.assigned_shipping_method = assigned_shipping_method
    checkout.shipping_method_name = assigned_shipping_method.name
