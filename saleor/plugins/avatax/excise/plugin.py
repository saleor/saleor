...
class AvataxExcisePlugin(BasePlugin):
    ...
    PLUGIN_ID = "mirumee.taxes.avalara.excise"
    ...
    def calculate_checkout_total(
        ...
        # Excise accounting requires additional notes to be passed in metadata
        checkout_obj.store_value_in_private_metadata(items=tax_response)
        checkout_obj.save()

        # currency is stored on individual tax lines in TransactionTaxes
        # if there are tax lines we take the currency of the first one,
        # assuming they are all the same
        if len(tax_response["TransactionTaxes"])>0:
