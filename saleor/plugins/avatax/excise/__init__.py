...
META_CODE_KEY = "avatax_excise.code"
META_DESCRIPTION_KEY = "avatax_excise.description"
CACHE_TIME = 60 * 60  # 1 hour
TAX_CODES_CACHE_TIME = 60 * 60 * 24 * 7  # 7 days
CACHE_KEY = "avatax_excise_request_id_"
TAX_CODES_CACHE_KEY = "avatax_excise_tax_codes_cache_key"
...
def api_get_request(url: str, config: AvataxExciseConfiguration):
    try:
        auth = HTTPBasicAuth(config.username_or_account, config.password_or_license)
        headers = {'x-company-id': config.company_name}
...
def _validate_order(order: "Order") -> bool:
    """Validate the order object if it is ready to generate a request to avalara excise."""
...
def _validate_checkout(checkout: "Checkout") -> bool:
    """Validate the checkout object if it is ready to generate a request to avalara excise."""
...
def generate_request_data(
    ...
    if company_address:
        ...
        logging.warning(
            "To correct calculate taxes by Avatax Excise, company address should be provided "
...
def _fetch_new_taxes_data(
    data: Dict[str, Dict], data_cache_key: str, config: AvataxExciseConfiguration
):
...
def get_order_tax_data(
    order: "Order", config: AvataxExciseConfiguration, force_refresh=False
) -> Dict[str, Any]:
...
def get_cached_tax_codes_or_fetch(
    config: AvataxExciseConfiguration, cache_time: int = TAX_CODES_CACHE_TIME
):
