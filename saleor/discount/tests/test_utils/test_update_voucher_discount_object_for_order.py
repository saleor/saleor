...
def test_update_voucher_discount_specific_product_with_apply_once_per_order(
    ...
    assert cheapest_line.total_price_net_amount == quantize_price(
        cheapest_line.unit_price_net_amount * cheapest_line.quantity,
        currency,
    )
    assert (
        cheapest_line.undiscounted_total_price_net_amount
        == cheapest_line.undiscounted_base_unit_price_amount * cheapest_line.quantity
    )
    ...
    assert order.shipping_price_net == shipping_price
    assert order.shipping_price_gross == shipping_price * tax_rate
    expected_subtotal_net = sum(line.total_price_net_amount for line in lines)
    expected_subtotal_gross = sum(line.total_price_gross_amount for line in lines)
    assert order.subtotal_net_amount == expected_subtotal_net
...
def test_update_voucher_discount_entire_order_with_shipping(
    ...
    assert order.subtotal_net_amount == sum(
        line.total_price_net_amount for line in lines
    )
