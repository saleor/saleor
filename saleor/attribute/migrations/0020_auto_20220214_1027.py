...
def import_attribute_values(attr_values_qs, Model):
    ...
    reference_slugs = attr_values_qs.values_list("slug", flat=True)
    reference_ids = {fetch_reference_id_from_slug(slug) for slug in reference_slugs}

    model_queryset = Model.objects.filter(pk__in=reference_ids)
