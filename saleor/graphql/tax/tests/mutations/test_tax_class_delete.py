...
from .....tax.error_codes import TaxClassDeleteErrorCode
...
def test_delete_default_tax_class(staff_api_client, permission_manage_taxes):
    ...
    # then
    content = get_graphql_content(response)
    errors = content["data"]["taxClassDelete"]["errors"]
    assert errors
    assert errors[0]["code"] == TaxClassDeleteErrorCode.CANNOT_DELETE_DEFAULT_CLASS.name
