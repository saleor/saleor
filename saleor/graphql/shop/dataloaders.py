...
class CollectionAttributeBySiteSettingsIdLoader(DataLoader):
    """Load collection attributes settings by site settings ID."""

    context_key = "collection_attributes_by_sitesettings"

    def batch_load(self, keys):
        def with_attributes(attribute_collections):
            site_settings_to_attributes_map = {}
            attribute_ids = []
            for site_settings, cat_attrs in zip(keys, attribute_collections):
                site_attr_ids = [cat_attr.attribute_id for cat_attr in cat_attrs]
                attribute_ids.extend(site_attr_ids)
                site_settings_to_attributes_map[site_settings] = site_attr_ids
        ...
        return (
            AttributeCollectionsBySiteSettingsIdLoader(self.context)
            .load_many(keys)
            .then(with_attributes)
        )
