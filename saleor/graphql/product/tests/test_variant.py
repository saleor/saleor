...
def test_product_variant_bulk_create_with_numeric_attribute(
    staff_api_client, product, numeric_attribute, permission_manage_products
):
    product_variant_count = ProductVariant.objects.count()
    product.product_type.variant_attributes.set([numeric_attribute])
    attribute_value_count = numeric_attribute.values.count()
    product_id = graphene.Node.to_global_id("Product", product.pk)
    attribute_id = graphene.Node.to_global_id("Attribute", numeric_attribute.pk)
    attribute_value_1 = numeric_attribute.values.first()
    attribute_value_2 = numeric_attribute.values.last()
    sku = str(uuid4())[:12]
    variants = [
        {
            "sku": sku,
