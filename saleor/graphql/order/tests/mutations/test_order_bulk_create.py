...
def test_order_bulk_create(
    ...
    assert db_order.user.addresses.count() == user_address_count
    assert db_order.draft_save_billing_address is None
    assert db_order.draft_save_shipping_address is None
...
def test_order_bulk_create_save_addresses_in_customer_book(
    staff_api_client,
    permission_manage_orders,
    permission_manage_orders_import,
    permission_manage_users,
    ...
    # given
    order = order_bulk_input
    # change the field in billing address to ensure that both shipping and billing
    # addresses are the new one
    order_bulk_input["billingAddress"]["postalCode"] = "00-000"
    order["saveShippingAddress"] = True
    order["saveBillingAddress"] = True
    ...
    # when the bulk order is called with save address options set
    response = staff_api_client.post_graphql(ORDER_BULK_CREATE, variables)
    content = get_graphql_content(response)

    # then the addresses are saved in customer address book
    assert content["data"]["orderBulkCreate"]["count"] == 1
    data = content["data"]["orderBulkCreate"]["results"]
    assert not data[0]["errors"]
    ...
    db_order = Order.objects.get()
    assert db_order.billing_address
    assert db_order.shipping_address
    assert db_order.draft_save_billing_address is None
    assert db_order.draft_save_shipping_address is None
    assert customer_user.addresses.count() == user_address_count + 2


def test_order_bulk_create_do_not_save_addresses_in_customer_book(
    staff_api_client,
    permission_manage_orders,
    permission_manage_orders_import,
    permission_manage_users,
    ...
    # addresses are the new one
    order_bulk_input["billingAddress"]["postalCode"] = "00-000"
    order["saveShippingAddress"] = False
    order["saveBillingAddress"] = False
    ...
    # when the bulk order is called with save address options set to False
    response = staff_api_client.post_graphql(ORDER_BULK_CREATE, variables)
    content = get_graphql_content(response)

    # then the addresses are not saved in customer address book
    assert content["data"]["orderBulkCreate"]["count"] == 1
    data = content["data"]["orderBulkCreate"]["results"]
    assert not data[0]["errors"]
...
def test_order_bulk_create_missing_shipping_address_save_shipping_addresses_provided(
    ...
    order_bulk_input["shippingAddress"] = None
    order["saveShippingAddress"] = save_shipping_address
    ...
    # then the error is assigned, order is not created
    assert content["data"]["orderBulkCreate"]["count"] == 0
    errors = content["data"]["orderBulkCreate"]["results"][0]["errors"]
    assert len(errors) == 1

    error0 = content["data"]["orderBulkCreate"]["results"][0]["errors"][0]
    assert error0["path"] == "save_shipping_address"
    assert error0["code"] == OrderBulkCreateErrorCode.MISSING_SHIPPING_ADDRESS_DATA.name


def test_order_bulk_create_save_addresses_set_missing_user_id(
    staff_api_client,
    permission_manage_orders,
    permission_manage_orders_import,
    permission_manage_users,
    ...
    order["saveShippingAddress"] = True
    order["saveBillingAddress"] = True
    order["user"] = {"email": "test-bulk@example.com"}
    ...
    # when the bulk order is called with save address options set and missing user
    response = staff_api_client.post_graphql(ORDER_BULK_CREATE, variables)
    content = get_graphql_content(response)
