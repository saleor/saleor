...
@patch(
    "saleor.graphql.order.mutations.draft_order_complete.fetch_order_prices_if_expired"
)
def test_draft_order_complete_prices_could_not_be_recalculated(
    mocked_fetch_order_prices,
    staff_api_client,
    permission_group_manage_orders,
    draft_order,
):
    # given
    # The recalculation did not store its result because a concurrent modification
    # intervened, so the order is still flagged when the mutation checks it.
    order = draft_order
    order.should_refresh_prices = True
    mocked_fetch_order_prices.return_value = Promise.resolve((order, order.lines.all()))
    ...
    # The order must stay a draft so it can still be recalculated and completed later.
    order.refresh_from_db()
    assert order.status == OrderStatus.DRAFT
...
def test_draft_order_complete_is_rejected_when_recalculation_collides(
    ...
    # given
    # This is the reported bug end to end: another process touches the order between
    # the untaxed base prices being written and the taxed prices being saved, so the
    # taxed save is skipped and the stored total carries no tax.
    order = draft_order
    order.should_refresh_prices = True
