...
def change_fail_event_type_up(apps, schema_editor):
    order_event = apps.get_model("order", "OrderEvent")

    refund_events = order_event.objects.filter(
        type=PAYMENT_FAILED,
        parameters__message__icontains="refund"
    )
    refund_events.update(
        type=PAYMENT_REFUND_FAILED
    )

    void_events = order_event.objects.filter(
        type=PAYMENT_FAILED,
        parameters__message__icontains="refund"
    )
    void_events.update(type=PAYMENT_VOID_FAILED)

    rest_events = (
        order_event.objects.
        filter(type=PAYMENT_FAILED).
        exclude(pk__in=refund_events.values_list("pk", flat=True)).
        exclude(pk__in=void_events.values_list("pk", flat=True))
    )

    rest_events.update(type=PAYMENT_CAPTURE_FAILED)


def change_fail_event_type_down(apps, schema_editor):
    order_event = apps.get_model("order", "OrderEvent")

    order_event.objects.filter(
        type=PAYMENT_REFUND_FAILED
    ).update(
        type=PAYMENT_FAILED
    )
