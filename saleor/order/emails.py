...
def _send_confirmation(email, url, template, ctx=None):
    site = Site.objects.get_current()
    context = {'site_name': site.name, 'url': url}
    if ctx:
        context.update(ctx)
    send_templated_mail(
...
@shared_task
def send_order_confirmation(email, url, order):
    ctx = {'order': order}
    _send_confirmation(email, url, CONFIRM_ORDER_TEMPLATE, ctx)
