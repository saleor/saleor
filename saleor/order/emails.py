...
def __send_confirmation(address, url, url_name, template):
...
def send_order_confirmation(address, url):
    __send_confirmation(address, url, 'order_url', CONFIRM_ORDER_TEMPLATE)


def send_payment_confirmation(address, url):
    __send_confirmation(address, url, 'payment_url', CONFIRM_PAYMENT_TEMPLATE)
