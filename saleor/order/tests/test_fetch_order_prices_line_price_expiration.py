...
def test_fetch_order_prices_lines_expired_base_prices(
    ...
    # expire order lines
    now = timezone.now()
    line_1.price_expired_at = now - timedelta(hours=1)
    line_2.price_expired_at = now - timedelta(hours=1)
    OrderLine.objects.bulk_update([line_1, line_2], ["price_expired_at"])
...
def test_fetch_order_prices_single_line_expired_base_prices(
    ...
    now = timezone.now()
    line_1.price_expired_at = now - timedelta(hours=1)
    line_1.save(update_fields=["price_expired_at"])
...
def test_fetch_order_prices_single_line_expired_manual_line_discount(
    ...
    now = timezone.now()
    line_2.price_expired_at = now - timedelta(hours=1)
    line_2.save(update_fields=["price_expired_at"])
