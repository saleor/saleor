        ...
        with allow_writer():
            with transaction.atomic(savepoint=False):
                ...
                if locked_order.updated_at == order.updated_at:
                    ...
                    # The prices calculated here were not stored, so the order keeps
                    # the ones from before the change that triggered this
                    # recalculation. Mirror that into the instance the caller holds,
                    # so it can tell that the stored prices are not the ones it got.
