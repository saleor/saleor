...
def prorate_line_total(stored_total, order_qty, quantity):
    if quantity == order_qty:
        return stored_total
    return (stored_total * quantity / order_qty).quantize(Decimal("0.01"))


def line_gross(order_line, quantity):
    return prorate_line_total(
        order_line.total_price_gross_amount, order_line.quantity, quantity
    )


def line_net(order_line, quantity):
    return prorate_line_total(
        order_line.total_price_net_amount, order_line.quantity, quantity
    )
...
def calculate_fulfillment_total(fulfillment):
    ...
    for line in fulfillment.lines.all():
        total += line_gross(line.order_line, line.quantity)
    return total
...
def _calculate_unfulfilled_weight(order, already_fulfilled_ids, new_fulfillments):
    ...
    for line in order.lines.all():
        ...
        if remaining_qty > 0:
            unfulfilled_total += line_gross(line, remaining_qty)
