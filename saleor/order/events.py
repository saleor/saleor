...
def draft_order_oversold_items_event(
    ...
    return _new_event(
        order=order, type=OrderEvents.OVERSOLD_ITEMS, user=user,
        parameters={
...
def order_canceled_event(*, order: Order, user: UserType):
    return _new_event(
        order=order, type=OrderEvents.CANCELED, user=user)
...
def payment_captured_event(
    ...
    return _new_event(
        order=order, type=OrderEvents.PAYMENT_CAPTURED, user=user,
...
def payment_refunded_event(
        *, order: Order, user: UserType,
        amount: Decimal, payment: Payment):
    return _new_event(
        order=order, type=OrderEvents.PAYMENT_REFUNDED, user=user,
        **_get_payment_data(amount, payment))


def payment_voided_event(*, order: Order, user: UserType, payment: Payment):
    return _new_event(
        order=order, type=OrderEvents.PAYMENT_VOIDED, user=user,
...
def payment_failed_event(
    ...
    return _new_event(
        order=order, type=OrderEvents.PAYMENT_FAILED, user=user,
...
def fulfillment_canceled_event(
    ...
    return _new_event(
        order=order, type=OrderEvents.FULFILLMENT_CANCELED, user=user,
...
def fulfillment_restocked_items_event(
    ...
    return _new_event(
        order=order, type=OrderEvents.FULFILLMENT_RESTOCKED_ITEMS, user=user,
        parameters={
...
def fulfillment_fulfilled_items_event(
    ...
    return _new_event(
        order=order, type=OrderEvents.FULFILLMENT_FULFILLED_ITEMS, user=user,
        parameters={
...
def fulfillment_tracking_updated_event(
        *, order: Order, user: UserType,
        ...
        fulfillment: Fulfillment):
    return _new_event(
        order=order, type=OrderEvents.TRACKING_UPDATED, user=user,
        parameters={
...
def order_note_added_event(*, order: Order, user: UserType, message: str):
    return _new_event(
        order=order, type=OrderEvents.NOTE_ADDED, user=user,
        parameters={
            'message': message})
...
def order_updated_address_event(
    ...
    return _new_event(
        order=order, type=OrderEvents.UPDATED_ADDRESS, user=user,
        parameters={
