...
@pytest.mark.e2e
def test_product_with_restricted_reference_attribute_core_0705(
    ...
    assert len(page_reference_attribute_data["referencePageTypes"]) == 1
    assert page_reference_attribute_data["referencePageTypes"][0]["id"] == page_type_id
    ...
    assert len(product_reference_attribute_data["referenceProductTypes"]) == 1
    assert (
        product_reference_attribute_data["referenceProductTypes"][0]["id"]
        == product_type_id
    )
    ...
    assert len(product_reference_attribute_data["referenceProductTypes"]) == 1
    assert (
        variant_reference_attribute_data["referenceVariantTypes"][0]["id"]
        == product_type_id
    )
    ...
    variant_id = create_product_variant(e2e_staff_api_client, product_not_in_choices_id)

    # Step 3 - Create product with a wrong attribute value and check for error
    invalid_attribute_data = [
        ...
        {"id": variant_ref_attribute_id, "reference": variant_id},
    ]
    ...
    # Step 4 - Create product with a correct attribute value
    ...
    correct_variant_ref = create_product_variant(
        e2e_staff_api_client, correct_product_ref["id"]
    )
    ...
    # Step 5 - Update attributes and clear reference types
    attribute_data = attribute_update(
        e2e_staff_api_client, page_ref_attribute_id, {"referenceTypes": []}
    )
    assert attribute_data["referencePageTypes"] == []

    attribute_data = attribute_update(
        e2e_staff_api_client, product_ref_attribute_id, {"referenceTypes": []}
    )
    assert attribute_data["referenceProductTypes"] == []

    attribute_data = attribute_update(
        e2e_staff_api_client, variant_ref_attribute_id, {"referenceTypes": []}
    )
    assert attribute_data["referenceVariantTypes"] == []

    # Step 6 - Update product with a previously wrong attribute value
    product_data = update_product(
        e2e_staff_api_client,
        product_id,
        {"attributes": invalid_attribute_data},
    )
    assert len(product_data["productUpdate"]["product"]["attributes"]) == len(
        invalid_attribute_data
    )
