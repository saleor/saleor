...
def test_login_throttling(
    ...
    # Step 2 - First attempt of loging with invalid credentials
    ...
    # Step 3 - Second attempt of loging with invalid credentials before it is allowed
    login_data = raw_token_create(
        e2e_not_logged_api_client, user_email, invalid_password
    )
    error = login_data["data"]["tokenCreate"]["errors"][0]
    ...
    # loging attempt counter and next allowed attempt time
    assert mocked_cache.get(block_key) == next_attempt
    assert mocked_cache.get(ip_key) == attempt_ip_counter
    assert mocked_cache.get(ip_user_key) == attempt_ip_user_counter

    # Step 4 - Third attempt of loging with invalid credentials when it is allowed
    ...
    # Step 5 - Third attempt of loging with invalid credentials when it is allowed and
    # username is unknown
    ...
    # Step 6 - Fourth attempt of loging with valid credentials when it is allowed
    mocked_cache.delete(block_key)
