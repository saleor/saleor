...
def test_order_products_on_fixed_promotion_CORE_2101(
    ...
    unit_price = float(variant_price) - float(discount)
    ...
    assert order_line["unitDiscount"]["amount"] == float(discount)
    assert order_line["unitDiscountType"] is not None
    assert order_line["unitDiscountValue"] == float(discount)
