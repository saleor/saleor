...
def order_line(order, variant):
    ...
    net = variant.get_price(product, [], channel_slug)
...
def order_with_lines(
    ...
    net = variant.get_price(product, [], channel_USD.slug)
    gross = Money(amount=net.amount * Decimal(1.23), currency=net.currency)
...
def order_with_lines_channel_PLN(
    ...
    net = variant.get_price(product, [], channel_PLN.slug)
    gross = Money(amount=net.amount * Decimal(1.23), currency=net.currency)
    line = order.lines.create(
        product_name=str(variant.product),
        variant_name=str(variant),
    ...
    net = variant.get_price(product, [], channel_PLN.slug, None)
    gross = Money(amount=net.amount * Decimal(1.23), currency=net.currency)
    line = order.lines.create(
        product_name=str(variant.product),
        variant_name=str(variant),
...
def allocations(order_list, stock, channel_USD):
    ...
    product = variant.product
    net = variant.get_price(product, [], channel_USD)
    gross = Money(amount=net.amount * Decimal(1.23), currency=net.currency)
