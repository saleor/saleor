...
def clone_attribute_data(original_attribute, new_attribute):
    ...
    for value in original_attribute.values.all():
        new_value = new_attribute.values.create(
            name=value.name, value=value.value, slug=value.slug,
...
def migrate_attributes_hstore_to_new_ids(apps, schema_editor):
    ...
    qs = Product.objects.select_related(
        'product_type').prefetch_related('variants')
    for product in qs:
...
def clean_stale_attributes(apps, schema_editor):
    ...
    Attribute.objects.filter(
        product_variant_type__isnull=True).filter(
            product_type__isnull=True).delete()


class Migration(migrations.Migration):

    dependencies = [
        ('product', '0073_auto_20181010_0729'),
    ]
    ...
    operations = [
        ...
        migrations.RunPython(
            migrate_attributes_hstore_to_new_ids,
