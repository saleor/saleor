...
def test_update_product_minimal_variant_price(product):
    ...
    variant.price = Money("4.99", "EUR")
    variant.save()
    product.refresh_from_db()

    assert product.minimal_variant_price == Money("10", "EUR")
...
def test_update_products_minimal_variant_prices_of_catalogues_for_product(product):
    variant = ProductVariant(
        product=product, sku="SKU_MINIMAL_VARIANT_PRICE", price=Money("0.99", "EUR"),
    )
    variant.save()
    product.refresh_from_db()
    assert product.minimal_variant_price == Money("10", "EUR")
...
def test_update_products_minimal_variant_prices_of_catalogues_for_category(
    ...
    variant = ProductVariant(
        product=product, sku="SKU_MINIMAL_VARIANT_PRICE", price=Money("0.89", "EUR"),
    )
    variant.save()
    product.refresh_from_db()
    assert product.minimal_variant_price == Money("10", "EUR")
...
def test_update_products_minimal_variant_prices_of_catalogues_for_collection(
    ...
    variant = ProductVariant(
        product=product, sku="SKU_MINIMAL_VARIANT_PRICE", price=Money("0.79", "EUR"),
    )
    variant.save()
    product.refresh_from_db()
...
def test_product_update_updates_minimal_variant_price(
    ...
    assert product.minimal_variant_price == Money("10.00", "EUR")
    query = """
...
def test_product_variant_objects_create_updates_minimal_variant_price(product):
    assert product.minimal_variant_price == Money("10.00", "EUR")
    ProductVariant.objects.create(product=product, sku="1", price=Money("1.00", "EUR"))
    product.refresh_from_db()
    assert product.minimal_variant_price == Money("1.00", "EUR")
...
def test_product_variant_objects_bulk_create_updates_minimal_variant_price(product):
    ...
    ProductVariant.objects.bulk_create(
        [
            ProductVariant(product=product, sku="1", price=Money("1.00", "EUR")),
            ProductVariant(product=product, sku="2", price=Money("5.00", "EUR")),
        ]
    )
    product.refresh_from_db()
    assert product.minimal_variant_price == Money("1.00", "EUR")
