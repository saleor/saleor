...
There are two backends defined for elasticsearch integration, `storefront <https://github/com/saleor/saleor/search/backends/elasticsearch.py>` and `dashboard <https://github.com/saleor/saleor/search/backends/elasticsearch_dashboard.py>`. Storefront search uses only storefront index for product only search, dashboard backend does additional searches in users and orders indexes as well.
