...
def test_get_purchased_items(order_with_lines, settings, voucher):
    ...
    assert len(payment.get_purchased_items()) == len(order_with_lines.get_lines())

    for p, o in zip(payment.get_purchased_items(), order_with_lines.get_lines()):
    ...
    assert len(payment.get_purchased_items()) == len(order_with_lines.get_lines()) + 1

    for p, o in zip(payment.get_purchased_items()[:-1], order_with_lines.get_lines()):
        assert p.sku == o.product_sku
        assert p.quantity == o.quantity
