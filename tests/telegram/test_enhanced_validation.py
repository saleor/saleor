...
def test_telegram_token_create_uniqueness():
    ...
    with override_settings(TELEGRAM_BOT_TOKEN="test_bot_token_123456789"):
        try:
            ...
            user1 = result1.user
            print(
                f"✅ First creation successful: User ID={user1.pk}, Email={user1.email}"
            )
            ...
            user2 = result2.user
            print(
                f"✅ Second creation successful: User ID={user2.pk}, Email={user2.email}"
            )

            # Verify if it's the same user
            if user1.pk == user2.pk:
                print(
                    "✅ User uniqueness validation passed: same telegram_id returns same user"
                )
                return True
            else:
                print(
                    "❌ User uniqueness validation failed: same telegram_id returns different users"
                )
                return False
...
def test_telegram_email_change_request_uniqueness():
    ...
    with override_settings(TELEGRAM_BOT_TOKEN="test_bot_token_123456789"):
        try:
            ...
            print(
                f"📝 Created conflicting user: User ID={conflicting_user.pk}, Email={conflicting_user.email}"
            )
            ...
            try:
                ...
                if email_result.errors:
                    print(
                        f"✅ Email uniqueness validation passed: detected conflicting email"
                    )
                    return True
                else:
                    print(
                        "❌ Email uniqueness validation failed: should have detected conflicting email"
                    )
                    return False

            except Exception as e:
                print(
                    f"✅ Email uniqueness validation passed: exception detected conflict - {e}"
                )
                return True
...
def test_telegram_user_metadata_consistency():
    ...
    with override_settings(TELEGRAM_BOT_TOKEN="test_bot_token_123456789"):
        try:
            ...
            # Validate key fields
            expected_fields = ["telegram_id", "created_via_telegram", "created_at"]
            ...
            for field in expected_fields:
                if field not in private_metadata:
                    ...
                    print(
                        f"✅ Metadata field exists: {field} = {private_metadata[field]}"
                    )

            # Validate telegram_id consistency
            if private_metadata["telegram_id"] != telegram_id:
                print(
                    f"❌ Telegram ID mismatch: expected={telegram_id}, actual={private_metadata['telegram_id']}"
                )
                return False
...
def main():
    ...
    tests = [
        (
            "Telegram Token Create User Uniqueness",
            test_telegram_token_create_uniqueness,
        ),
        (
            "Telegram Email Change Request Email Uniqueness",
            test_telegram_email_change_request_uniqueness,
        ),
        (
            "Telegram Email Change Confirm Email Uniqueness",
            test_telegram_email_change_confirm_uniqueness,
        ),
