...
def test_view_menu_list_assign_new_menu_to_settings(admin_client, menu, site_settings):
...
def test_view_menu_item_create_with_parent(admin_client, menu, menu_item, category):
    url = reverse(
        "dashboard:menu-item-add", kwargs={"menu_pk": menu.pk, "root_pk": menu_item.pk}
    )
    linked_object = str(category.id) + "_Category"
...
def test_view_menu_item_edit(admin_client, menu, menu_item, category):
    url = reverse(
        "dashboard:menu-item-edit", kwargs={"menu_pk": menu.pk, "item_pk": menu_item.pk}
    )
    linked_object = str(category.id) + "_Category"
...
def test_view_menu_item_delete_with_parent(admin_client, menu, menu_item):
    new_menu_item = MenuItem.objects.create(
        menu=menu, name="New Link", url="http://example.com/", parent=menu_item
    )
    url = reverse(
        "dashboard:menu-item-delete",
...
def test_view_ajax_reorder_menu_items(admin_client, menu, menu_with_items):
    ...
    url = reverse("dashboard:menu-items-reorder", kwargs={"menu_pk": menu.pk})
...
def test_view_ajax_reorder_menu_items_with_parent(admin_client, menu, menu_with_items):
...
def test_view_ajax_menu_links(admin_client, collection, category, page):
    ...
    category_repr = {"id": str(category.pk) + "_" + "Category", "text": str(category)}
    page_repr = {
