...
def test_token_create_user_data(
        permission_edit_order, staff_client, staff_user):
    query = """
    mutation {
        tokenCreate(email: "%(email)s", password: "%(password)s") {
            user {
    ...
    permission = permission_edit_order
...
def test_staff_create(
        admin_api_client, user_api_client, permission_edit_user,
        permission_edit_product, staff_user):
    query = """
    ...
    permission_edit_product_codename = '%s.%s' % (
        permission_edit_product.content_type.app_label,
        permission_edit_product.codename)

    email = 'api_user@example.com'
    staff_user.user_permissions.add(permission_edit_user)
    variables = json.dumps({
        'email': email, 'permissions': [permission_edit_product_codename]})
    ...
    assert permissions[0]['code'] == permission_edit_product_codename
