...
- Smoke-test a GraphQL operation without hand-rolling `curl | jq | python3` pipelines: `make gql QUERY='...'` (or `FILE=<path>`, or pipe via stdin), with optional `VARS='{...}'`. The helper runs `../saleor-platform/scripts/gql`, which re-uses `api-token` under the hood and pretty-prints the JSON response. Use this while iterating on a new mutation before you touch the dashboard side. In a `make` recipe, `$` in a GraphQL variable reference must be escaped as `$$` (e.g. `QUERY='query($$id: ID!){ page(id:$$id){ title }}'`).
- Reset demo state between rehearsals with `make prepare DEMO=<demo-id>`. This re-runs the demo's seed scripts, which are restorative (built with `update_or_create`, and they wipe any `(copy) …` artifacts produced by the feature under test). `make demo-reset-pages` exists as a low-level helper that only wipes copies — it does **not** restore the source Page, so prefer `make prepare` when you're iterating on a mutation and mutating seeded rows.
- After adding or changing a GraphQL mutation, run `make refresh-schema` from `saleor-demo` so the dashboard codegen picks up the new fields before you edit `../saleor-dashboard/src/**/mutations.ts`. The script calls `docker exec ... python manage.py get_graphql_schema` inside the api container (annotation-safe; matches Saleor CI) and writes the SDL to both `saleor/graphql/schema.graphql` and `../saleor-dashboard/schema-main.graphql`, then regenerates the TypeScript hooks.
